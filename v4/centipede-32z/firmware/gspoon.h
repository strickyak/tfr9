...
struct addr_byte __not_in_flash("coco2_reset") Coco2StartupGPokes[] = {
    // PIA0
...
struct addr_byte __not_in_flash("gime_reset") gime_reset_sequence[] = {
...
void IN_RAM SpoonfeedConsoleOnReset() {
...
#if FOR_COCO3
  for (auto *p = gime_reset_sequence; p->a; p++) {
    GPoke1(p->a, p->b);
  }
#endif

  for (auto *p = Coco2StartupGPokes; p->a; p++) {
    GPoke1(p->a, p->b);
  }
...
#if SAM_BIT_64K
  // In my coco-zoo, I see: coco1,64k: $FF22 <- $03 ; coco2,16k: $FF22 <= $07
  byte low_bits = GPeek1(0xFF22) & 0x03;
#else
...
#endif
#if SAM_BIT_64K
  GPoke1(0xFFDD, 42);  // set M1 for 64k
#endif
  ...
  GPoke1(0xFFCB, 42);  // Framebuffer at 0x0800
