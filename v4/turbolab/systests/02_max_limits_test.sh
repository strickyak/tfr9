#!/bin/bash
# 02_max_limits_test.sh - Test --max cycle, time, and watchpoint limits + core dump
set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/common.sh"

echo "Running Suite 2: Execution Limits Tests (--max)"

# 1. Test Decimal SI Cycle Limit (c:10k)
log_test "Cycle limit with decimal SI suffix (--max=c:10k)"
run_tether -n --max=c:10k "$BUILD_DIR/turbos/turbos_core.img"
...
log_test "Cycle limit with binary multiplier (--max=c:16K = 16384 cycles)"
run_tether -n --max=c:16K "$BUILD_DIR/turbos/turbos_core.img"
...
log_test "Wall-clock duration limit (--max=t:300ms)"
run_tether -n --max=t:300ms "$BUILD_DIR/turbos/turbos_core.img"
...
log_test "Watchpoint limit on entry address (--max=x:0x1000)"
run_tether -n --max=x:0x1000 "$DATA_DIR/bootable/pi.decb"
...
log_test "Watchpoint limit with Nth count (--max=x:0x1019:5)"
run_tether -n --max=x:0x1019:5 "$DATA_DIR/bootable/pi.decb"
assert_contains "$TETHER_STDERR" "Watchpoint Limit Reached at Cycle #101 (Addr=\$1019" "Halted on the 5th execution of \$1019"
assert_exit_code 0 $TETHER_EXIT "Tether exited cleanly"

# 6. Test Watchpoint Limit with Symbolic OS-9 Module Offset (--max=@kernel+0x1B)
log_test "Watchpoint limit on module symbol (--max=@kernel+0x1B)"
run_tether -n --max=@kernel+0x1B "$BUILD_DIR/turbos/turbos_core.img"
assert_contains "$TETHER_STDERR" "Resolved max watchpoint @kernel+0x1B to \$E3CC" "Resolved @kernel+0x1B to \$E3CC"
