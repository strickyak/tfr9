...
run_tether -n --max=c:50 "$DATA_DIR/bootable/pi.decb"
...
run_tether -n --trace=x --max=c:10 "$DATA_DIR/bootable/pi.decb" "$DATA_DIR/bootable/pi.decb.list"
...
run_tether -n --max=c:50 "$BUILD_DIR/turbos/turbos_core.img"
...
run_tether -n --max=c:10 -listings "$LISTINGS_DIR" "$BUILD_DIR/turbos/turbos_dev.img.rom"
