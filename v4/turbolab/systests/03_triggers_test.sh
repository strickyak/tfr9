...
log_test "Cycle trigger (--trigger=c:60 --stop=c:100 --trace=x)"
run_tether -n --trigger=c:60 --stop=c:100 --trace=x "$DATA_DIR/bootable/pi.decb"
...
log_test "Watchpoint trigger with Nth count (--trigger=x:0x1019:3 --stop=c:120 --trace=x)"
run_tether -n --trigger=x:0x1019:3 --stop=c:120 --trace=x "$DATA_DIR/bootable/pi.decb"
...
log_test "Symbolic module trigger (--trigger=@kernel+0x1B --stop=c:30 --trace=x)"
run_tether -n --trigger=@kernel+0x1B --stop=c:30 --trace=x "$BUILD_DIR/turbos/turbos_core.img"
