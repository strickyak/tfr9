...
run_tether -n --trace=x --max=c:50 "$DATA_DIR/bootable/pi.decb"
...
run_tether -n --trace=+ --max=c:50 "$DATA_DIR/bootable/pi.decb"
...
run_tether -n --trace=w --max=c:100 "$DATA_DIR/bootable/pi.decb"
...
run_tether -n --trace=1 --max=c:100 "$DATA_DIR/bootable/pi.decb"
assert_matches_regex "$TETHER_STDERR" "^x [0-9A-Fa-f]{4} " "Emitted opcode fetch (x) records"
assert_matches_regex "$TETHER_STDERR" "^\+ [0-9A-Fa-f]{4} " "Emitted continuation (+) records"
assert_matches_regex "$TETHER_STDERR" "^w [0-9A-Fa-f]{4} " "Emitted write (w) records"
assert_exit_code 0 $TETHER_EXIT "Tether exited cleanly"
...
run_tether -n --trace=x --max=c:50 "$DATA_DIR/bootable/pi.decb" "$DATA_DIR/bootable/pi.decb.list"
...
run_tether -n --trace=x --max=c:20 "$BUILD_DIR/turbos/turbos_core.img"
