...
/*
#define PIOV   volatile pio_hw_t *
#define pio0v  ((volatile pio_hw_t *)PIO0_BASE)

static inline uint32_t fast_pio_sm_get_blocking(PIOV pio, uint sm) {
     //fast//check_pio_param(pio);
     //fast//check_sm_param(sm);
...
static inline void fast_pio_sm_put(PIOV pio, uint sm, uint32_t data) {
     //fast// check_pio_param(pio);
     //fast// check_sm_param(sm);
     pio->txf[sm] = data;
}
...
void RunCPU() {
    ...
    while (true) {
        ...
        while (true) {
            ...
            pio_sm_put(pio0, 0, 0xBD);  // $BD=JSR  // z77
            ...
            if (rounds > 3) switch (step) {
                ...
                case 7:
                        ...
                        if (countdown == 0) {
                            ...
                            // WHY DO WE SEE THIS:
                            // @ s7 got fdbd want ffff xor 0242
