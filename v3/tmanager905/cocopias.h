...
constexpr const char* KB_NORMAL = "@ABCDEFGHIJKLMNOPQRSTUVWXYZ\204\205\206\207 0123456789:;,-./\r\014\033\000\000\201\202\000";
constexpr const char* KB_SHIFT = "`abcdefghijklmnopqrstuvwxyz____ 0!\"#$%&'()*+<=>?___..__.";
constexpr const char* KB_CTRL = ".................................|.~...^[]..{_}\\........";
...
static byte Keyboard_ComputeSense(byte probe, byte ch, LOGGER logf) {
	...
	logf("Keyboard_ComputeSense: probe $%x char $%x sense $%x shifted $%x", probe, ch, sense, shifted);
...
struct DoCocoKeyboard {
  ...
  static bool Keyboard_CanRx() {
      return (CocoKeyboardState == CKS_Empty);
  }
  static void Keyboard_SetRx(uint ch) {
      CocoKeyboardChar = ch;
...
struct DontCocoPias {
  ...
  static bool VsyncIrqEnabled() {
      return false;
  }
  //bool vsync_irq_firing;
  static bool VsyncIrqFiring() {
      return false;
  }
...
struct DoCocoPias {
  ...
  static bool VsyncIrqEnabled() {
      return Pia0.Cb1IrqEnabled();
  }
  //bool vsync_irq_firing;
  static bool VsyncIrqFiring() {
      return Pia0.Cb1IrqFiring();
  }
